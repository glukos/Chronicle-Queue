...
class StoreAppender extends AbstractCloseable
        implements ExcerptAppender, ExcerptContext, SingleChronicleQueueExcerpts.InternalAppender, ReferenceOwner {
    @NotNull
    ...
    StoreAppender(@NotNull final SingleChronicleQueue queue,
        ...
        this.closableResources = new SingleChronicleQueueExcerpts.ClosableResources<>(this, storePool);
    ...
    public void checkReleased() {
        if (!isClosed()) throw new IllegalStateException("Not closed");
    ...
    private void resetPosition() throws UnrecoverableTimeoutException {
        try {
            ...
            assert !SingleChronicleQueueExcerpts.CHECK_INDEX || checkPositionOfHeader(bytes);

            final long headerNumber = store.lastSequenceNumber(this);
            wire.headerNumber(queue.rollCycle().toIndex(cycle, headerNumber + 1) - 1);

            assert !SingleChronicleQueueExcerpts.CHECK_INDEX || wire.headerNumber() != -1 || checkIndex(wire.headerNumber(), positionOfHeader);
        ...
        assert !SingleChronicleQueueExcerpts.CHECK_INDEX || checkWritePositionHeaderNumber();
    }
    ...
    protected void writeBytesInternal(final long index, @NotNull final BytesStore bytes, boolean metadata) {
        ...
        if (rollbackDontClose) {
            ...
            SingleChronicleQueueExcerpts.LOG.warn("Trying to overwrite index {} which is before the end of the queue", Long.toHexString(index));
            return;
        }
    ...
    class StoreAppenderContext extends AbstractCloseable implements DocumentContext {
