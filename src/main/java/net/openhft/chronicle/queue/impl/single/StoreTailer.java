...
class StoreTailer extends AbstractCloseable
        implements ExcerptTailer, SourceContext, ExcerptContext, ReferenceOwner {
    ...
    private final SingleChronicleQueueExcerpts.ClosableResources<?> closableResources;
    ...
    public StoreTailer(@NotNull final SingleChronicleQueue queue, final LongValue indexValue) {
        ...
        closableResources = new SingleChronicleQueueExcerpts.ClosableResources<>(this, queue);
    ...
    private static MessageHistory readHistoryFromBytes(@NotNull final Wire wire, final MessageHistory history) {
        final Bytes<?> bytes = wire.bytes();
        if (SingleChronicleQueueExcerpts.MESSAGE_HISTORY_METHOD_ID != wire.readEventNumber())
            return null;
    ...
    private static MessageHistory readHistoryFromWire(@NotNull final Wire wire, final MessageHistory history) {
        final StringBuilder sb = SingleChronicleQueueExcerpts.SBP.acquireStringBuilder();
    ...
    public void close() {
        ...
        if (w0 != null)
            SingleChronicleQueueExcerpts.releaseIfNotNullAndReferenced(w0.bytes(), this);
        wireForIndex = null;
        if (w0 != null)
    ...
    private long nextIndexWithNextAvailableCycle(final int cycle) {
        ...
        if (SingleChronicleQueueExcerpts.LOG.isDebugEnabled()) {
            ...
            if (nextIndex != Long.MIN_VALUE && nextIndexCycle - 1 != cycle) {
                ...
                SingleChronicleQueueExcerpts.LOG.debug("Rolled " + (nextIndexCycle - cycle) + " " + "times to find the " +
    ...
    private boolean headerNumberCheck(@NotNull final AbstractWire wire) {
        ...
        wire.headNumberCheck((actual, position) -> {
            try {
                ...
                if (actual == expecting)
                    return true;
                SingleChronicleQueueExcerpts.LOG.error("", new AssertionError("header number check failed " +
                        "expecting=" + expecting +
                        "  !=  actual=" + actual));

                return false;
            } catch (Exception e) {
                SingleChronicleQueueExcerpts.LOG.error("", e);
                return false;
            }
        });
    ...
    private void resetWires() {
        ...
        assert !SingleChronicleQueueExcerpts.CHECK_INDEX || headerNumberCheck(wire);
        ...
        assert !SingleChronicleQueueExcerpts.CHECK_INDEX || headerNumberCheck((AbstractWire) wireForIndex);
        ...
        switch (direction) {
            ...
            case FORWARD:
                ...
                if (rollCycle.toSequenceNumber(seq) < seq) {
                    cycle(cycle + 1);
                    SingleChronicleQueueExcerpts.LOG.warn("we have run out of sequence numbers, so will start to write to " +
    ...
    class StoreTailerContext extends BinaryReadDocumentContext {
        ...
        public void wire(@Nullable final AbstractWire wire) {
            ...
            if (oldWire != null) {
                SingleChronicleQueueExcerpts.releaseWireResources(oldWire);
            }
        }
    }
}
